...
function Layout() {
  ...
  {profileOpen && token && (
    <div className="absolute right-0 mt-3 w-72 rounded-2xl border border-white/10 bg-slate-950 p-4 shadow-xl">
      ...
      >
        ...
        {mobileOpen &&profileOpen && token && (
          <div className="absolute right-4 top-16 z-20 w-72 rounded-2xl border border-white/10 bg-slate-950 p-4 shadow-xl">
            <div className="mb-3 flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-slate-800 text-white">
                {user?.name ? user.name.slice(0, 2).toUpperCase() : 'U'}
              </div>
